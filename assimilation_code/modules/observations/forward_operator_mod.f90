...
module forward_operator_mod
   ...
   use obs_sequence_mod,      only : init_obs, destroy_obs, obs_sequence_type, &
      ...
      get_obs_def, get_obs_from_key, replace_obs_values, get_obs_key
...
!------------------------------------------------------------------------------
   subroutine get_obs_ens_distrib_state(ens_handle, obs_fwd_op_ens_handle, &
      ...
      type(obs_sequence_type), intent(inout)    :: seq  !! the observation sequence
...
!------------------------------------------------------------------------------
   subroutine get_expected_obs_distrib_state(seq, keys, state_time, isprior, &
      ...
      integer :: num_obs, i
      ! A.D'A
      real(r8), DIMENSION(1) :: obs_value
      real(r8)               :: updated_error_variance, amf_ratio, obs_err_var
      integer :: key_value
      num_obs = size(keys)
...
!>@todo do you ever use this with more than one obs?
      do i = 1, num_obs
         ...
         if(obs_kind_ind < 0) then
            ...
            ! A.D'A
            call get_obs_values(obs, obs_value)
            key_value = get_obs_key(obs)
