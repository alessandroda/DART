    ...
    def before_step(self):
        ...
        for dict_mem in dict_mem_list: #loop on mems
            ...
            if self.restart_from_controlrun and self.time_manager.current_time == self.time_manager.start_time:
                ...
                safe_symlink(self.path_manager.chimere2023_END_FILE_SRC(self.control_run_exp_name, end_file_date_control_run.strftime("%Y%m%d")), 
                             self.path_manager.chimere2023_END_FILE(dict_mem["MemberID"], end_file_date_control_run.strftime("%Y%m%d00"), 24))
                self.current_end_file[dict_mem["MemberID"]]=self.path_manager.chimere2023_END_FILE(dict_mem["MemberID"], end_file_date_control_run.strftime("%Y%m%d00"), 24)
            else:
        ...
        for filename in os.listdir(f"{self.path_manager.path_filter}"):
            if filename.startswith("analysis_"):
                ...
                try:
                    ...
                    logger.info(f"Moved '{filename}' to '{self.path_manager.dart_preassim_dir(date_ymdH)}'")
                except shutil.Error:
