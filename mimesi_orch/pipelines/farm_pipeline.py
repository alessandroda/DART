...
class FarmToDartPipeline(BaseAssimilationPipeline):
    def __init__(
        self,
        time_manager,
        path_manager,
        ...
        self.run_assimilation_flag = self.config["assimilation"][
            "run_assimilation_flag"
        ]
        ...
        if current_day != self.time_manager.last_perturbed_day:
            ...
            for mem in range(self.no_mems):
                path_emi_mem = (
                    self.path_manager.base_path
                    / self.path_manager.path_data
                    / f'INPUT/HERMES/emi_{mem}/HERMESv3_{days_back.strftime("%Y%m%d00")}.nc'
                )
        ...
        for mem in range(self.no_mems):
            try:
                ...
                for date_backward in dates_backward:
                    file_path = (
                        mem_path / f'ic_g1_{date_backward.strftime("%Y%m%d%H")}.nc'
                    )
                    if file_path.exists():
    ...
    def replace_perturb_into_original_emissions(self):
        ...
        """
        logger.info(
            f"Replacing perturbated emissions into original HERMES file in {self.path_manager.base_path / self.path_manager.path_data / 'INPUT/HERMES/'}"
        )
        logger.warning(
        ...
        original_path = (
            self.path_manager.base_path
            / self.path_manager.run_submit_replace_perturbations
        )
        output_nml_path = original_path.with_name(
            f"{original_path.stem}{date_start_end}{original_path.suffix}"
        )
        replace_nml_template(
        ...
        while True:
            ...
            for jobid in job_ids:
                if not check_job_status_cresco(
                    jobid, which_run=f"{original_path.name}"
                ):
