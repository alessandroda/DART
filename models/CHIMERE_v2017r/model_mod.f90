...
! for the public routines.
   ...
   use        location_mod,     only : location_type, get_close_type, get_dist, query_location, get_location, &
      ...
      set_location, set_location_missing, set_vertical_localization_coord, &
   ...
   use netcdf_utilities_mod, only : nc_add_global_attribute, nc_synchronize_file, &
      ...
      nc_check, nc_close_file, nc_dimension_exists
...
! Use the namelist for options to be set at runtime.
   character(len=256) :: chimere_template_filename = '/mnt/mumbai_n3r6/25-12676_MIMESI/kAiros/out.2025121500_2025121600_ITA7.nc'
   integer  :: time_step_days      = 0
   integer  :: time_step_seconds   = 3600
   integer, parameter :: MAX_NUM_STATE_VARIABLES = 30
   integer, parameter :: MAX_NUM_COLUMNS = 5
   character(len=32)  :: vertical_localization_coord = 'LEVEL'
...
!> where the variable data is located and what size it is.
   ...
   type chimere_grid
      type(chimere_1d_array) :: lons
      type(chimere_1d_array) :: lats
      type(chimere_1d_array) :: levs
      !corner points
      real :: west
      real :: south
      real :: east
      ...
      real :: delta_lon
      real :: delta_lat
   end type chimere_grid

   type(chimere_grid) :: grid_data
...
! Record the namelist values used for the run
      ...
      call get_CHIMERE_grid(chimere_template_filename)

      call set_calendar_type('GREGORIAN')

      ! state_time = read_model_time(tiegcm_restart_file_name)

      call set_chimere_variable_info(chimere_template_filename, state_variables)
...
!------------------------------------------------------------------
   ...
   subroutine get_state_meta_data(index_in, location, var_type)
      ...
      location = set_location(real(grid_data%lons%vals(iloc), r8), real(grid_data%lats%vals(jloc), r8), grid_data%levs%vals(vloc), vertical_localization_type)
...
! Flush the buffer and leave netCDF file open
   ...
   subroutine fill_chimere_2d_as_1d(ncid, varname, grid_array)
      integer,            intent(in)    :: ncid
      character(len=*),   intent(in)    :: varname
      type(chimere_1d_array), intent(inout) :: grid_array

      integer :: nlat, nlon
      real(r8), allocatable :: tmp(:,:)

      character(len=*), parameter :: routine = 'fill_chimere_2d_as_1d'

      if (.not. nc_variable_exists(ncid, varname)) return

      nlat = nc_get_dimension_size(ncid, 'south_north', routine)
      nlon = nc_get_dimension_size(ncid, 'west_east',   routine)

      allocate(tmp(nlat, nlon))
      call nc_get_variable(ncid, varname, tmp, routine)

      grid_array%nsize = nlat * nlon
      allocate(grid_array%vals(grid_array%nsize))

      grid_array%vals = reshape(tmp, [grid_array%nsize])

      deallocate(tmp)
   end subroutine

   subroutine fill_chimere_vert_array(ncid, dimname, grid_array)
      ...
      integer :: dimid, i
      integer :: nsize
      character(len=*), parameter :: routine = 'fill_chimere_vert_array'

      ! !------------------------------------------------------------
      ! ! Check dimension exists
      ! !------------------------------------------------------------
      if (.not. nc_dimension_exists(ncid, dimname)) then
         write(string1,*) 'Vertical dimension ', trim(dimname), ' not found'
         call error_handler(E_ERR, routine, string1, source, revision, revdate)
      endif

      !------------------------------------------------------------
      ! Get dimension size (number of vertical levels)
      !------------------------------------------------------------
      grid_array%nsize = nc_get_dimension_size(ncid, dimname, routine)

      allocate(grid_array%vals(grid_array%nsize))

      !------------------------------------------------------------
      ! Optionally fill with indices (1..nlev) or leave empty
      !------------------------------------------------------------
      grid_array%vals = [(i, i=1, grid_array%nsize)]

   end subroutine fill_chimere_vert_array



   !------------------------------------------------------------------------------------------------------------------
   !> Read the data from the various CHIMERE grid arrays
   subroutine get_CHIMERE_grid(file_name)
      ...
      integer  :: ierr
      real(r8), allocatable :: lat2d(:,:), lon2d(:,:)

      character(len=*), intent(in) :: file_name

      character(len=*), parameter :: routine = 'get_CHIMERE_grid'

      call error_handler(E_MSG,routine,'reading grid from ['//trim(file_name)//']')
      ...
      nlat = nc_get_dimension_size(ncid, 'south_north', routine)
      nlon = nc_get_dimension_size(ncid, 'west_east',   routine)
      nlev = nc_get_dimension_size(ncid, 'bottom_top',           routine)
      ...
      call nc_get_variable(ncid, 'lon', lon2d)
      call nc_get_variable(ncid, 'lat', lat2d)

      ! --- longitude convention: [-180,180] -> [0,360]
      where (lon2d < 0.0_r8)
         lon2d = lon2d + 360.0_r8
      end where

      ! --- flatten lon/lat for DART
      grid_data%lons%nsize = nlon
      allocate(grid_data%lons%vals(nlon))
      grid_data%lons%vals = lon2d(:,1)

      grid_data%lats%nsize = nlat
      allocate(grid_data%lats%vals(nlat))
      grid_data%lats%vals = lat2d(1,:)      ! --- grid extents
      ...
      ! --- vertical levels (1D)
      call fill_chimere_vert_array(ncid, 'bottom_top', grid_data%levs)

      ! --- grid spacing (assumes quasi-regular grid)
   ...
   end subroutine get_CHIMERE_grid
...
!>@param nfields         the number of variable/Quantity pairs specified
   ...
   subroutine set_chimere_variable_info(kairos_template_filename, variable_array)

      character(len=*), intent(in)  :: kairos_template_filename
      character(len=*), intent(in)  :: variable_array(:)

      character(len=*), parameter :: routine = 'set_chimere_variable_info:'
      ...
      dom_id = add_domain(kairos_template_filename, nfields, var_names, kind_list, &
         clamp_vals, update_list)

   end subroutine set_chimere_variable_info


   !--------------------------------------------------------------------
   !> Locate the horizontal grid cell containing a given geographic point.
   !>
   !> Given a target latitude and longitude, this routine finds the
   !> indices (i,j) of the horizontal grid cell in which the point lies.
   !> The grid is assumed to be:
   !>   - logically rectangular
   !>   - defined by 1D latitude and longitude coordinate arrays
   !>   - monotonic in both latitude and longitude
   !>
   !> The cell is defined by:
   !>   lat(i) <= obs_lat <= lat(i+1)
   !>   lon(j) <= obs_lon <= lon(j+1)
   !>
   !> No interpolation is performed: the routine only identifies the
   !> enclosing grid cell. This is suitable for nearest-cell or
   !> cell-centered state representations.
   !>
   !> If the point lies outside the grid boundaries, `cell_found` is
   !> returned as `.false.` and `cells` is not modified.
   !>
   !> Assumptions / limitations:
   !>   - Latitude and longitude arrays must be strictly increasing
   !>   - Grid spacing is assumed quasi-regular
   !>   - Longitude wrap-around (e.g. dateline crossing) is NOT handled
   !>
   !> @param obs_lat    Latitude of the target location
   !> @param obs_lon    Longitude of the target location
   !> @param lat        1D array of grid latitude coordinates
   !> @param lon        1D array of grid longitude coordinates
   !> @param cells      Output (i,j) indices of the enclosing grid cell
   !>                   cells(1) = latitude index
   !>                   cells(2) = longitude index
   !> @param cell_found Logical flag indicating whether a valid cell was found
   !--------------------------------------------------------------------
   subroutine find_cell(obs_lat, obs_lon, lat, lon, cells, cell_found)
      ...
      ! ---------------------------------------
      cells(1) = 1
      dmin = abs(lat(1) - obs_lat)

      do i = 2, size(lat)
         if (abs(lat(i) - obs_lat) < dmin) then
            dmin = abs(lat(i) - obs_lat)
            cells(1) = i
         end if
      end do
      ...
      ! ---------------------------------------
      cells(2) = 1
      dmin = abs(lon(1) - obs_lon)

      do j = 2, size(lon)
         if (abs(lon(j) - obs_lon) < dmin) then
            dmin = abs(lon(j) - obs_lon)
            cells(2) = j
         end if
      end do
...
!--------------------------------------------------------------------
   ...
   function read_model_time(filename)
      ...
      integer :: ncid, time_dimlen, times_varid
...
! ------------------------------------------------------------
      ...
      allocate(times(time_dimlen))

! ------------------------------------------------------------
! Read Times variable
! ------------------------------------------------------------
      call nc_check( nf90_inq_varid(ncid, 'Times', times_varid), &
         routine, 'inquire Times varid' )

      call nc_check( nf90_get_var(ncid, times_varid, times), &
         routine, 'read Times variable' )

! ------------------------------------------------------------
! Parse FIRST time string: YYYY-MM-DD_HH:MM:SS
! ------------------------------------------------------------
      read(times(time_dimlen)(1:4),  *) year
      read(times(time_dimlen)(6:7),  *) month
      read(times(time_dimlen)(9:10), *) day
      read(times(time_dimlen)(12:13),*) hour
      read(times(time_dimlen)(15:16),*) minute
      read(times(time_dimlen)(18:19),*) second

      utsec = hour*3600 + minute*60 + second

      read_model_time = set_date(year, month, day) + set_time(utsec, 0)

! ------------------------------------------------------------
! Debug output (optional)
! ------------------------------------------------------------
      if (do_output()) then
         write(*,*) trim(filename)//':read_model_time: Times(1) = ', trim(times(1))
         call print_date(read_model_time, str='read_model_time: date ')
         call print_time(read_model_time, str='read_model_time: time ')
      endif
...
!> vertical type we are going to want to localize in.
   ...
   subroutine set_vert_localization(typename)
      ...
      select case (ucasename)
       ...
       case ("PRESSURE", "HEIGHT", "SCALEHEIGHT", "SCALE_HEIGHT", "SCALE HEIGHT")

         write(string1,*) 'CHIMERE-DART currently supports ONLY MODEL LEVEL localization.'
         write(string2,*) 'You requested "', trim(typename), &
            '", which is NOT implemented.'
         write(string3,*) 'Forcing vertical localization to LEVEL.'
       ...
       case default
         ...
         write(string1,*) 'Unrecognized vertical localization type: "', trim(typename), '"'
         write(string2,*) 'Valid value for CHIMERE is: LEVEL'
         write(string3,*) 'Forcing vertical localization to LEVEL.'
      ...
      ! Set localization coordinate used internally by DART
