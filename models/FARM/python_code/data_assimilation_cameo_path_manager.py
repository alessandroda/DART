...
while t <= end_time:
    ...
    output_sim_folder = (
        path_manager.base_path / f'models/FARM/projects/CAMEO/posteriors/{formatted_t_str}'
    )
    Path(output_sim_folder).mkdir(parents=True, exist_ok=True)
    analysis_sim_folder = (
        path_manager.base_path / f'models/FARM/projects/CAMEO/analysis/{formatted_t_str}'
    )
    Path(analysis_sim_folder).mkdir(parents=True, exist_ok=True)
    preassim_sim_folder = (
        path_manager.base_path / f'models/FARM/projects/CAMEO/preassim/{formatted_t_str}'
    )
    Path(preassim_sim_folder).mkdir(parents=True, exist_ok=True)

    # FILTER INPUT NML
    replace_nml_template(
        path_manager.base_path / "models/FARM/work/input_template.nml",
        entries_tbr_dict={
        ...
        output_nml_path= path_manager.base_path / "models/FARM/work/input.nml",
    )

    # # Define paths to files
    # meteo_file = path_manager.base_path / "CAMEO/RUN/data/INPUT/METEO/ifsecmc"
    # output_meteo_1hr = path_manager.base_path / "CAMEO/RUN/data/INPUT/METEO/met_1hr.nc"
    # output_meteo_1hr_plus1 = path_manager.base_path / "CAMEO/RUN/data/INPUT/METEO/met_1hr+1.nc"
    # conc_file = path_manager.base_path / "CAMEO/RUN/data/OUTPUT/conc_g1_"
    ...
    # # Step 3: Shift the time by 1 hour
    ...
    # arconv_input = path_manager.base_path / "path/to/your/farm_input_file"
    # arconv_output = path_manager.base_path / "path/to/your/arconv_output_file"
    # subprocess.run(["arconv", arconv_input, arconv_output, "1"])

    # # Step 5: Use ncks to append P, SP, and T variables to the FARM concentration file
    # subprocess.run(["ncks", "-A", "-v", "P,SP,T", output_meteo_1hr_plus1, conc_file])

    # Tranform te comment into code
    ...
    replace_nml_template(
        path_manager.base_path / "models/FARM/work/filter_input_list_template.txt",
        entries_tbr_dict={
            "$days": str(seconds_model),
            "$seconds": str(days_model),
        },
        output_nml_path=path_manager.base_path / "models/FARM/work/filter_input_list.txt",
    )

    # FILTER_OUTPUT_LIST.TXT
    replace_nml_template(
        path_manager.base_path / "models/FARM/work/filter_output_list_template.txt",
        entries_tbr_dict={
            "$folder_path": output_sim_folder,
        },
    ...
    replace_nml_template(
        ...
        output_nml_path=path_manager.path_submit_bsh / "templates/submit_filter.bsh",
    )
    replace_nml_template(
        path_manager.path_submit_bsh / "templates/run_filter.template.bsh",
        entries_tbr_dict={
            "NO_PROC": str(20)
        },
        output_nml_path=path_manager.path_submit_bsh / "templates/run_filter.bsh",
    )
    # RUN THE FILTER
    job_id = run_command_in_directory_bsub("./submit_filter.sh", path_manager.path_submit_bsh)
    import pdb; pdb.set_trace()
