...
class Settings(BaseSettings):
    ...
    class Config:
        env_prefix = "BC_"  # Allow overriding settings with environment variables
...
def perturb_emission():
    ...
    for netcdf_emi in netcdfs:
        ...
        nx, ny, nz = (
            len(emission_dataset[settings.var].x),
            len(emission_dataset[settings.var].y),
        ...
        grid_length = get_dist(
            emission_dataset[settings.var].y[0],
            emission_dataset[settings.var].y[1],
            emission_dataset[settings.var].x[0],
            emission_dataset[settings.var].y[1],
        )
        weights_dict = compute_weights(nx, ny, settings.corr_length_hz,
                grid_length, *np.meshgrid(emission_dataset.y,
                    emission_dataset.x))
