...
def make_obs_vs_data(settings, dataset_variable, dataset, station_points):
    ...
    for i, time_step in enumerate(dataset.time):
        ...
        formatted_str = str(time_step.values)
        ...
        for lon, lat in zip(filtered_time_points["lon"], filtered_time_points["lat"]):
            ...
            filtered_time_points.loc[
                (filtered_time_points["lat"] == lat)
                & (filtered_time_points["lon"] == lon),
                "model",
            ] = model_value
        ...
        # valid_indices = ~np.isnan(values_obs) & ~np.isnan(values_model)
        # values_obs = values_obs[valid_indices]
        # values_model = values_model[valid_indices]

        plt.figure(figsize=(8, 6))
        plt.scatter(
            values_obs, values_model, color="blue", label="Observed vs. Model Data"
        )
        ...
        print(r2_score(values_obs, values_model))
...
class Settings(BaseModel):
    ...
    path_netcdf: str = (
        "/mnt/mumbai_n4r5/dausilio/projects/DART/models/FARM/IRIDE_ens/3D_plots/_analysis_avg.nc"
    )
...
def make_single_pngs(
    ...
    for layer_i in settings.layers:
        ...
        image_filename = os.path.join(
            ...
            f"{settings.title}_{layer_i}_{settings.model}.png",
        )
...
def main():
    ...
    dirlist = []
    for dir in os.listdir(path_dirlist):
        if dir.endswith("00"):
            dirlist.append(pd.to_datetime(dir, format="%Y%m%d_%H%M%S"))
    # filter station_points on dirlist times
    station_y_points = station_points[station_points.index.isin(dirlist)]

    daily_mean = (
        station_y_points.groupby(["EoIcode", pd.Grouper(freq="D")])[
            ["value", "lon", "lat"]
        ]
        .mean()
        .reset_index()
    ...
    station_y_points = (
        daily_mean.set_index("datetime")
        .groupby("EoIcode")
        .resample("Y")
        .mean()
        .reset_index()
    )
    station_y_points = station_y_points[station_y_points["value"] > 0]
    # make_obs_vs_data(
    #     settings=settings,
    #     dataset=dataset,
    ...
    filenames = make_single_pngs(
        ...
        dataset=dataset,
        dataset_variable=subset_variable,
        station_points=station_y_points,
    )
