...
! ! obs_def_SAT_HCHO_TROPOMI_mod
! ! author : Alessandro D'Ausilio
! ! email : a.dausilio@aria-net.it
!****************************************************************************************************
! This section defines the forward operator for assimilating S5P Tropomi HCHO in
! FARM. The 'preprocess' function utilizes this operator to incorporate appropriate
! definitions of SAT_HCHO_TROPOMI in the DEFAULT_obs_def_mod.f90 template. Subsequently, it generates
...
! The subroutine 'get_expected_SAT_HCHO_TROPOMI' is employed by the filter and performs the A*V operation.
...
! kernel and pressure vectors via set_obs_def_hcho_tropomi. It subsequently calls 'write_tropomi_hcho'
! to write the observations into the observation sequence file.
!
! During filter execution, the code initially passes through 'read_tropomi_hcho' to read previously set
! and written Tropomi HCHO observations, performing interpolation.
!****************************************************************************************************


! BEGIN DART PREPROCESS TYPE DEFINITIONS
! SAT_HCHO_TROPOMI, QTY_HCHO
! END DART PREPROCESS TYPE DEFINITIONS

! BEGIN DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE
!   use obs_def_SAT_HCHO_TROPOMI_mod, only : get_expected_SAT_HCHO_TROPOMI, write_tropomi_hcho, read_tropomi_hcho, &
!   set_obs_def_hcho_tropomi
! END DART PREPROCESS USE OF SPECIAL OBS_DEF MODULE


! BEGIN DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF
!   case(SAT_HCHO_TROPOMI)
!       call get_expected_SAT_HCHO_TROPOMI(state_handle, ens_size, location, obs_def%key, obs_value, amf_ratio, expected_obs, istatus)
! END DART PREPROCESS GET_EXPECTED_OBS_FROM_DEF

! BEGIN DART PREPROCESS READ_OBS_DEF
!         case(SAT_HCHO_TROPOMI)
!           call read_tropomi_hcho(obs_def%key, ifile, fileformat)
! END DART PREPROCESS READ_OBS_DEF

! BEGIN DART PREPROCESS WRITE_OBS_DEF
!         case(SAT_HCHO_TROPOMI)
!           call write_tropomi_hcho(obs_def%key, ifile, fileformat)
! END DART PREPROCESS WRITE_OBS_DEF

! BEGIN DART PREPROCESS set_obs_def_hcho_tropomi
!      case(SAT_HCHO_TROPOMI)
!         call set_obs_def_hcho_tropomi(obs_def%key)
! END DART PREPROCESS set_obs_def_hcho_tropomi
...
module obs_def_SAT_HCHO_TROPOMI_mod
   ...
   use     obs_kind_mod, only : QTY_HCHO, QTY_PRESSURE, QTY_SURFACE_PRESSURE
   ...
   public ::  get_expected_SAT_HCHO_TROPOMI, write_tropomi_hcho,read_tropomi_hcho, set_obs_def_hcho_tropomi
...
! linear steps in pressure between the surface and top.
   ...
   namelist /obs_def_SAT_HCHO_TROPOMI_nml/ unit_conversion
...
! Read the namelist entry
      call find_namelist_in_file("input.nml", "obs_def_SAT_HCHO_TROPOMI_nml", iunit)
      read(iunit, nml = obs_def_SAT_HCHO_TROPOMI_nml, iostat = rc)
      call check_namelist_read(iunit, rc, "obs_def_SAT_HCHO_TROPOMI_nml")

! Record the namelist values used for the run ...
      if (do_nml_file()) write(nmlfileunit, nml=obs_def_SAT_HCHO_TROPOMI_nml)
      if (do_nml_term()) write(     *     , nml=obs_def_SAT_HCHO_TROPOMI_nml)

   end subroutine initialize_module


!------------------------------------------------------------------------------
   subroutine get_expected_SAT_HCHO_TROPOMI(state_handle, ens_size, location, key, obs_sat, amf_ratio, val, istatus)
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
...
!     FARM surface pressure field at pixel position
      ...
      model_levels_conc: do
         ...
         locS = set_location(mloc(1),mloc(2),farm_heights(nz),VERTISHEIGHT)
         call interpolate(state_handle, ens_size, locS, QTY_HCHO, model_conc(:, nz), int_conc_status)
   ...
   end subroutine get_expected_SAT_HCHO_TROPOMI

   subroutine read_tropomi_hcho(key, ifile, fform)
      ...
      counts1 = counts1 + 1
      key = counts1
      call set_obs_def_hcho_tropomi(key, avg_kernels_1,obs_p_1, amf_1)

   end subroutine read_tropomi_hcho


   subroutine write_tropomi_hcho(key, ifile, fform)
      integer, intent(in) :: key
      integer, intent(in)  :: ifile
      character(len=*),  intent(in), optional :: fform
...
11       format('pressure_px', i8)
   ...
   end subroutine write_tropomi_hcho

   subroutine set_obs_def_hcho_tropomi(key, kernel_trop_px_1, pressure_px_1, amf_1)
      integer, intent(in) :: key
      integer :: i
   ...
   end subroutine set_obs_def_hcho_tropomi
...
!------------------------------------------------------------
   ...
   real function UnitConversion_ugm3Pa_molm2(varin)
      ...
      real, parameter     ::  Mw = 30.031     ! g_tr/mol_tr
      ! begin ----------------------------------
...
end module obs_def_SAT_HCHO_TROPOMI_mod
